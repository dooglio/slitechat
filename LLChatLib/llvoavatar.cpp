...
#include "noise.h"
#include "Agent.h" //  Get state values from here
...
BOOL	LLViewerJointMesh::sDisableLOD = FALSE;
...
// setValid()
//--------------------------------------------------------------------
void LLViewerJointMesh::setValid( BOOL valid, BOOL recursive )
{
	//----------------------------------------------------------------
...
void LLViewerJointMesh::setMeshesToChildren()
{
	removeAllChildren();
	for (LLViewerJointList::iterator iter = mMeshParts.begin();
...
{
	LLVector3d camera_pos_global = m_agent->getCameraPositionGlobal();
...
const LLVector3 LLVOAvatar::getRenderPosition() const
{
	...
	//return mRegion->getPosAgentFromRegion( getPosition() );
	return LLVector3();
}
		...
		)
{
	...
	if (mIsSelf && !m_agent->needsRenderAvatar() || !LLPipeline::sPickAvatar)
	{
		return FALSE;
	}
...
BOOL LLVOAvatar::setupBone(const LLVOAvatarBoneInfo* info, LLViewerJoint* parent, S32 &volume_num, S32 &joint_num)
{
	...
	if (parent)
	{
		parent->addChild( static_cast<LLJoint*>(joint.get()) );
	}
	...
	for (iter = info->mChildList.begin(); iter != info->mChildList.end(); iter++)
	{
		...
		if (!setupBone(child_info, joint.get(), volume_num, joint_num))
		{
			return FALSE;
		}
	}
...
void LLVOAvatar::restoreMeshData()
{
	...
	if (mIsSelf)
	{
		updateAttachmentVisibility(m_agent->getCameraMode());
	}
	else
	{
		for (attachment_map_t::iterator iter = mAttachmentPoints.begin(); 
										  ...
										  LLDataPacker *dp)
{
	...
	{
		if(this == m_agent->getAvatarObject())
		{
			//tell sim to cancel this update
			m_agent->teleportViaLocation(m_agent->getPositionGlobal());
		}
	}
...
void LLVOAvatar::idleUpdateVoiceVisualizer(bool voice_enabled)
{
	...
	mVoiceVisualizer->setVoiceEnabled( voice_enabled && !(mIsSelf && m_agent->cameraMouselook()) );
	if ( voice_enabled )
	{		
		//----------------------------------------------------------------
...
void LLVOAvatar::idleUpdateAppearanceAnimation()
{
	...
	if (mAppearanceAnimating)
	{
		...
		if (appearance_anim_time >= APPEARANCE_MORPH_TIME)
		{
			...
			if (mIsSelf)
			{
				m_agent->sendAgentSetAppearance();
			}
		}
		else
		{
...
void LLVOAvatar::idleUpdateTractorBeam()
{
	...
	else if (!mBeam || mBeam->isDead())
	{
		...
		mBeam->setColor(LLColor4U(m_agent->getEffectColor()));
	...
	if (!mBeam.isNull())
	{
		...
		if (m_agent->mPointAt.notNull())
		{
			// get point from pointat effect
			mBeam->setPositionGlobal(m_agent->mPointAt->getPointAtPosGlobal());
			mBeam->triggerLocal();
		}
...
{
	m_agent->setPositionAgent(getPositionAgent());
...
//------------------------------------------------------------------------
BOOL LLVOAvatar::updateCharacter( LLC::Agent& agent )
{
	LLMemType mt(LLMemType::MTYPE_AVATAR);
...
//		llinfos << "Setting timestep to " << time_quantum * pixel_area_scale << llendl;
	...
	if (!(mIsSitting && getParent()))
	{
		...
		if (mIsSelf)
		{
			m_agent->setPositionAgent(getRenderPosition());
		}

		root_pos = m_agent->getPosGlobalFromAgent(getRenderPosition());
		...
		LLVector3 newPosition = m_agent->getPosAgentFromGlobal(root_pos);
		...
		if (!isAnyAnimationSignaled(AGENT_NO_ROTATE_ANIMS, NUM_AGENT_NO_ROTATE_ANIMS))
		{
			...
			if (mIsSelf && m_agent->cameraMouselook())
			{
				// make sure fwdDir stays in same general direction as primdir
				if (m_agent->getFlying())
				{
				...
				else
				{
					...
					LLVector3 up_vector = m_agent->getReferenceUpVector();
			...
			BOOL self_in_mouselook = mIsSelf && m_agent->cameraMouselook();
			...
			if (mIsSelf && mTurning)
			{
				...
				{
					m_agent->setControlFlags(AGENT_CONTROL_TURN_RIGHT);
				}
				else
				{
					m_agent->setControlFlags(AGENT_CONTROL_TURN_LEFT);
				}
			}
...
//							AUDIO_STEP_LO_GAIN, AUDIO_STEP_HI_GAIN );
			...
			LLVector3d foot_pos_global = m_agent->getPosGlobalFromAgent(foot_pos_agent);
...
U32 LLVOAvatar::renderSkinned(EAvatarRenderPass pass)
{
	...
	if (LLViewerShaderMgr::instance()->getVertexShaderLevel(LLViewerShaderMgr::SHADER_AVATAR) <= 0)
	{
		if (mNeedsSkin)
		{
			...
			if (!mIsSelf || m_agent->needsRenderHead() || LLPipeline::sShadowRender)
			{
	...
	if (sDebugInvisible)
	{
		...
		else if (!m_agent->needsRenderAvatar())
		{
	...
	if (mIsSelf && !m_agent->needsRenderAvatar())
	{
		return num_indices;
	}
...
U32 LLVOAvatar::renderRigid()
{
	...
	if (mIsSelf && (!m_agent->needsRenderAvatar() || !m_agent->needsRenderHead()))
	{
		return 0;
	}
...
void LLVOAvatar::resolveHeightAgent(const LLVector3 &in_pos_agent, LLVector3 &out_pos_agent, LLVector3 &out_norm)
{
	...
	in_pos_global = m_agent->getPosGlobalFromAgent(in_pos_agent);
	resolveHeightGlobal(in_pos_global, out_pos_global, out_norm);
	out_pos_agent = m_agent->getPosAgentFromGlobal(out_pos_global);
}
...
void LLVOAvatar::resolveHeightGlobal(const LLVector3d &inPos, LLVector3d &outPos, LLVector3 &outNorm)
{
	...
	else
	{
		...
		LLVector3 relativePos = m_agent->getPosAgentFromGlobal(outPos) - obj->getPositionAgent();
...
BOOL LLVOAvatar::processSingleAnimationStateChange( const LLUUID& anim_id, BOOL start )
{
	...
	if ( start ) // start animation
	{
		if (anim_id == ANIM_AGENT_TYPE)
		{
			if (gAudiop)
			{
				LLVector3d char_pos_global = m_agent->getPosGlobalFromAgent(getCharacterPosition());
...
void LLVOAvatar::stopMotionFromSource(const LLUUID& source_id)
{
	...
	{
		m_agent->sendAnimationRequest( motion_it->second, ANIM_REQUEST_STOP );
...
LLJoint* LLVOAvatar::findCollisionVolume(U32 volume_id)
{
	...
	return mCollisionVolumes[volume_id].get();
}
...
LLVector3 LLVOAvatar::getCharacterVelocity()
{
	...
	return mVelocity - mStepObjectVelocity;
}
...
void LLVOAvatar::getGround(const LLVector3 &in_pos_agent, LLVector3 &out_pos_agent, LLVector3 &outNorm)
{
	...
	p0_global = m_agent->getPosGlobalFromAgent(in_pos_agent) + z_vec;
	p1_global = m_agent->getPosGlobalFromAgent(in_pos_agent) - z_vec;
#if 0
	LLViewerObject *obj;
	...
	out_pos_agent = m_agent->getPosAgentFromGlobal(out_pos_global);
#endif
	out_pos_agent = m_agent->getPosAgentFromGlobal( mPosition );
}
...
{
	return m_agent->getPosGlobalFromAgent(position);
}
...
{
	return m_agent->getPosAgentFromGlobal(position);
}
...
BOOL LLVOAvatar::allocateCharacterJoints( U32 num )
{
	...
	mNumJoints = 0;
	mSkeleton.clear();
	mSkeleton.reserve( num );

	for(S32 joint_num = 0; joint_num < (S32)num; joint_num++)
	{
		LLViewerJointPtr ptr( new LLViewerJoint() );
		mSkeleton.push_back( ptr );
		ptr->setJointNum(joint_num);
	}

	mNumJoints = (int) mSkeleton.size();
	return TRUE;
}
...
BOOL LLVOAvatar::allocateCollisionVolumes( U32 num )
{
...
#if 0
	mCollisionVolumes = new LLViewerJoint[num]; //LLViewerJointCollisionVolume[num];
	...
	for( U32 joint_num = 0; joint_num < num; ++joint_num )
	{
		LLViewerJointPtr ptr( new LLViewerJoint() );
		mCollisionVolumes.push_back( ptr );
		ptr->setJointNum(joint_num);
	}
#endif
...
// getCharacterJoint()
//-----------------------------------------------------------------------------
LLViewerJointPtr LLVOAvatar::getCharacterJoint( U32 num )
{
	if ((S32)num >= mNumJoints 
	    || (S32)num < 0)
	{
		return LLViewerJointPtr();
	}
...
void LLVOAvatar::requestStopMotion( LLMotion* motion )
{
	...
	if ( mIsSelf )
	{
		...
		//m_agent->requestStopMotion( motion );
	}
}
...
{
	mRoot.addChild( (LLJoint*) mSkeleton[0].get() );
...
BOOL LLVOAvatar::updateJointLODs()
{
...

	{
		...
		if (mIsSelf)
		{
			if(m_agent->cameraCustomizeAvatar() || m_agent->cameraMouselook())
			{
...
void LLVOAvatar::updateShadowFaces()
{
	...
	if (mShadowImagep->getHasGLTexture())
	{
		...
		if (mFootLeftp)
		{
			...
			resolveRayCollisionAgent(m_agent->getPosGlobalFromAgent(joint_world_pos), 
				m_agent->getPosGlobalFromAgent(gSky.getSunDirection() + joint_world_pos), shadow_pos, normal);
			shadow_pos_agent = m_agent->getPosAgentFromGlobal(shadow_pos);
			...
			if (mIsSelf && m_agent->getCameraMode() == CAMERA_MODE_MOUSELOOK)
			{
...
void LLVOAvatar::sitOnObject(LLViewerObject *sit_object)
{
	...
	if (mIsSelf)
	{
		...
		m_agent->setFlying(FALSE);
		m_agent->setThirdPersonHeadOffset(LLVector3::zero);
		//interpolate to new camera position
		m_agent->startCameraAnimation();
		// make sure we are not trying to autopilot
		m_agent->stopAutoPilot();
		m_agent->setupSitCamera();
		if (m_agent->mForceMouselook) m_agent->changeCameraToMouselook();
	}
}
...
void LLVOAvatar::getOffObject()
{
	...
	if (mIsSelf)
	{
		LLQuaternion av_rot = m_agent->getFrameAgent().getQuaternion();
		...
		at_axis.mV[VZ] = 0.f;
		at_axis.normalize();
		m_agent->resetAxes(at_axis);

		//reset orientation
//		mRoot.setRotation(avWorldRot);
		m_agent->setThirdPersonHeadOffset(LLVector3(0.f, 0.f, 1.f));

		m_agent->setSitCamera(LLUUID::null);
	}
}
	...
	if (success)
	{
		...
		if (self)
		{
			   ...
			   discard_level < local_tex_data.mDiscard)
			{
				...
				if ( self->isSelf() && !m_agent->cameraCustomizeAvatar() )
				{
					self->requestLayerSetUpdate( index );
				}
				else if( self->isSelf() && m_agent->cameraCustomizeAvatar() )
				{
...
void LLVOAvatar::processRebakeAvatarTextures(LLMessageSystem* msg, void**)
{
	...
	LLVOAvatar* self = m_agent->getAvatarObject();
	if (!self) return;
...
void LLVOAvatar::setLocTexTE( U8 te, LLViewerImage* image, BOOL set_by_user )
{
	...
	if( m_agent->cameraCustomizeAvatar() )
	{
		LLVisualParamHint::requestHintUpdates();
	}
}
...
void LLVOAvatar::updateMeshTextures()
{
	...
	const BOOL self_customizing = mIsSelf && m_agent->cameraCustomizeAvatar(); // During face edit mode, we don't use baked textures
		...
		if (tex->getID() != IMG_DEFAULT_AVATAR)
		{
			if (local_tex_data.mDiscard > desired_discard)
			{
				...
				if (tex_discard >= 0 && tex_discard <= desired_discard)
				{
					...
					if( mIsSelf && !m_agent->cameraCustomizeAvatar() )
					{
						requestLayerSetUpdate( index );
					}
					else if( mIsSelf && m_agent->cameraCustomizeAvatar() )
					{
						LLVisualParamHint::requestHintUpdates();
					}
				}
...
{
	LLVOAvatar *avatarp = m_agent->getAvatarObject();
	if (avatarp)
	{
...
{
	LLVOAvatar* avatar = m_agent->getAvatarObject();
	LLAPRFile outfile ;
			...
			if (items.count())
			{
				...
				for (S32 i = 0; i < items.count(); i++)
				{
					...
					if ( item_permissions.allowOperationBy(
								PERM_MODIFY, m_agent->getID(), m_agent->getGroupID()) &&
						 item_permissions.allowOperationBy(
								PERM_COPY, m_agent->getID(), m_agent->getGroupID()) &&
						 item_permissions.allowOperationBy(
								PERM_TRANSFER, m_agent->getID(), m_agent->getGroupID()) )
					{
						can_grab = TRUE;
						break;
					}
