...
typedef boost::shared_ptr<LLViewerJointMesh> LLViewerJointPtr;
//
class LLViewerJointMesh : public LLJoint
{
public:
			LLViewerJointMesh()
				: m_meshId(-1)
	...
	void		setMeshID( S32 id ) 			{ m_meshId = id;	}
	S32			getMeshID() const				{ return m_meshId;	}
	//
	void		setIsTransparent( bool val )	{ m_isTransparent = val;  }
	...
	//
	void		setMesh( LLPolyMesh* mesh ) { m_mesh = mesh; }
	LLPolyMesh*	getMesh() const { return m_mesh; }
	...
	F32 getLOD() { return m_minPixelArea; }
	void setLOD( F32 pixelArea ) { m_minPixelArea = pixelArea; }

	//BOOL updateLOD(F32 pixel_area, BOOL activate);

	// Sets the shininess
	void setSpecular( const LLColor4& color, F32 shiny ) { /*mSpecular = color;*/ m_shiny = shiny; };
	//
	void							setPickName(LLVOAvatarDictionary::PickName name)	{ m_pickName = name; }
	LLVOAvatarDictionary::PickName	getPickName()										{ return m_pickName; }

	void setMeshesToChildren();

	typedef std::vector<LLViewerJointMesh*> LLViewerJointList;
	LLViewerJointList					mMeshParts;
...
private:
	LLColor4							m_color;			// color value
	F32									m_shiny;			// shiny value
	S32									m_meshId;
	F32									m_minPixelArea;
	bool								m_isTransparent;
	bool								mValid;
	LLVOAvatarDictionary::PickName		m_pickName;
	LLPolyMesh*							m_mesh;
	//
	std::vector<LLViewerJointPtr> m_meshParts;
};

typedef LLViewerJointMesh	LLViewerJoint;
//
class LLPolyMesh : public LLViewerJointMesh
{
public:
	LLPolyMesh()			{}
	virtual ~LLPolyMesh()	{}
...
{
public:
	LLVOAvatar(const LLUUID &id, const LLPCode pcode, LLViewerRegionPtr regionp, LLC::Agent* agent);
	virtual ~LLVOAvatar();
...
public:
	...
	LLViewerRegionPtr					getRegion() const						{ return mRegion; }
	void								setRegion( LLViewerRegionPtr region )	{ mRegion = region; }
...
public:
	...
	virtual LLViewerJointPtr getCharacterJoint( U32 num );
...
public:
	...
	BOOL updateCharacter(LLC::Agent &agent);
...
public:
	...
	LLJointAttachment* getTargetAttachmentPoint(LLViewerObject* viewer_object);
...
#endif
	bool			hasPendingBakedUploads() { return false; }
...
public:
	...
	typedef std::vector<LLViewerJointPtr> LLViewerJointList;
	LLViewerJointList	mCollisionVolumes;
...
public:
	...
	typedef std::map<S32, LLJointAttachment*> attachment_map_t;
...
private:
	...
	LLJoint *mScreenp; // special purpose joint for HUD attachments
...
private:
	struct BakedTextureData
	{
		...
		std::vector< LLViewerJoint * > mMeshes;  // std::vector<LLViewerJoint> mJoints[i]->mMeshParts
	};
