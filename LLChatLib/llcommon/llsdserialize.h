...
#	include <tinyjson/tinyjson_deprecated.hpp>
#else
...
private:
	typedef json::grammar<char> grammar_t;
	typedef grammar_t::variant	variant_t;
	typedef grammar_t::object	object_t;
	typedef object_t::iterator	iterator_t;

	S32 parseObject( object_t obj, LLSD& data ) const;
};
...
#if 0
	// TODO
	static S32 toJSON(const LLSD& sd, std::ostream& str)
	{
		LLPointer<LLSDJSONFormatter> f = new LLSDJSONFormatter;
		return f->format(sd, str, LLSDFormatter::OPTIONS_NONE);
	}
#endif
	static S32 fromJSON(LLSD& sd, std::istream& str )
	{
		LLPointer<LLSDJSONParser> p = new LLSDJSONParser;
		return p->parse(str, sd, LLSDSerialize::SIZE_UNLIMITED );
	}
	static LLSD fromJSON(std::istream& str )
	{
		LLPointer<LLSDJSONParser> p = new LLSDJSONParser;
		LLSD sd;
		(void)p->parse(str, sd, LLSDSerialize::SIZE_UNLIMITED );
		return sd;
	}
};
