								   ...
								   const LLSD& input) const
{
    ...
    //std::cout << "LLTrustedMessageService::post: name=" << name.c_str() << std::endl;
							...
							const LLSD& context, const LLSD& input) const
{
    ...
    //std::cout << "LLMessageHandlerBridge::post: name=" << name.c_str() << std::endl;
...
LLMessageSystem* LLMessageSystem::mInstance = NULL;

LLMessageSystem* LLMessageSystem::getInstance()
{
	if( mInstance == NULL )
	{
		throw std::runtime_error( "You must initialize first using InitMessageSystem()!" );
	}
	//
	return mInstance;
}

void LLMessageSystem::Release()
{
	delete mInstance;
	mInstance = NULL;
}

void LLMessageSystem::InitMessageSystem(const std::string& filename, U32 port, 
								 S32 version_major,
								 S32 version_minor,
								 S32 version_patch,
								 ...
								 const F32 circuit_heartbeat_interval, const F32 circuit_timeout)
{
	...
	mInstance = new LLMessageSystem( filename, port, 
								 version_major,
								 version_minor,
								 version_patch,
								 failure_is_fatal,
							 ...
							 iter != end; iter++)
						{
								 ...
								 iter != end; iter++)
							{
								...
								for( S32 blocknum = 0; blocknum < getNumberOfBlocksFast( block_name ); blocknum++ )
								{
									...
									LLMsgBlkDataPtr msg_block_data = mCurrentRMessageData->mMemberBlocks[bnamep];
...
void LLMessageSystem::disableCircuit(const LLHost &host)
{
	...
	U32 code = mInstance->findCircuitCode( host );
	...
	if (code)
	{
		...
		U64 ip_port = 0;
		std::map<U32, U64>::iterator iter = mInstance->mCircuitCodeToIPPort.find(code);
		if (iter != mInstance->mCircuitCodeToIPPort.end())
		{
			ip_port = iter->second;

			mInstance->mCircuitCodeToIPPort.erase(iter);
			...
			mInstance->mIPPortToCircuitCode.erase(ip_port);
		}
											...
											void** user)
{
	...
	if (circuit_code_in)
	{
		...
		if (!had_circuit_already)
		{
			...
			//
			cdp->checkPacketInID(mInstance->mCurrentRecvPacketID, FALSE ); // Since this is the first message on the circuit, by definition it's not resent.
		}
...
{
	if ((mInstance->mMessageTemplates.find
			(LLMessageStringTable::getInstance()->getString(msg_name.c_str())) ==
				mInstance->mMessageTemplates.end()) &&
		!LLMessageConfig::isValidMessage(msg_name))
	{
		std::cout << "Ignoring unknown message " << msg_name.c_str() << std::endl;
	...
	if (!handler)
	{
		std::cout	<< "LLMessageService::dispatch > no handler for "
				<< path.c_str() << std::endl;
		return;
	}
	// enable this for output of message names
...
{
	LLMessageSystem::InitMessageSystem(
		template_name,
		port, 
		version_major, 
		version_minor, 
...
{
	LLTransferManager::getInstance()->cleanup();
	LLTransferManager::Release();
...
U64 LLMessageSystem::getMessageTimeUsecs(const BOOL update)
{
	if (mInstance)
	{
		if (update)
		{
			mInstance->mCurrentMessageTimeSeconds = totalTime()*SEC_PER_USEC;
		}
		return (U64)(mInstance->mCurrentMessageTimeSeconds * USEC_PER_SEC);
	}
	else
	{
		return totalTime();
...
F64 LLMessageSystem::getMessageTimeSeconds(const BOOL update)
{
	if (mInstance)
	{
		...
		return mInstance->mCurrentMessageTimeSeconds;
	}
	else
	{
