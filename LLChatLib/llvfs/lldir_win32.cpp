...
LLDir_Win32::LLDir_Win32()
{
	...
	mOSUserDir = utf16str_to_utf8str(llutf16string((const U16*) w_str));
	...
	mOSCacheDir = utf16str_to_utf8str(llutf16string((const U16*) w_str));
	...
	if (GetTempPath(MAX_PATH, w_str))
	{
		...
		mTempDir = utf16str_to_utf8str(llutf16string((const U16*) w_str));
	}
	else
	{
		mTempDir = mOSUserDir;
...
//	fprintf(stderr, "mTempDir = <%s>",mTempDir);
	...
	{
		w_str[size] = '\0';
		mExecutablePathAndName = utf16str_to_utf8str(llutf16string((const U16*) w_str));
		...
		mWorkingDir = utf16str_to_utf8str(llutf16string((const U16*) w_str));
	...
	else
	{
		...
		GetCurrentDirectory(MAX_PATH, w_str);
		mExecutableDir = utf16str_to_utf8str(llutf16string((const U16*) w_str));
...
U32 LLDir_Win32::countFilesInDir(const std::string &dirname, const std::string &mask)
{
	...
	if ((count_search_h = FindFirstFile((const WCHAR *)pathname.c_str(), &FileData)) != INVALID_HANDLE_VALUE)   
	{
		file_count++;
...
BOOL LLDir_Win32::getNextFileInDir(const llutf16string &dirname, const std::string &mask, std::string &fname, BOOL wrap)
{
	...
	if (pathname != mCurrentDir)
	{
		...
		if ((mDirSearch_h = FindFirstFile((const WCHAR*) pathname.c_str(), &FileData)) == INVALID_HANDLE_VALUE)   
		{
...
//				llinfos << "Unable to locate next file" << llendl;
	...
	fname = utf16str_to_utf8str((const U16 *) FileData.cFileName);
...
//	llinfos << "Random select mp3 #" << which_file << llendl;
	...
	if ((random_search_h = FindFirstFile((const WCHAR*) pathname.c_str(), &FileData)) != INVALID_HANDLE_VALUE)   
	{
		while (which_file--)
		{
		...
		fname = utf16str_to_utf8str(llutf16string((const U16*) FileData.cFileName));
	}
}
...
std::string LLDir_Win32::getCurPath()
{
	...
	return utf16str_to_utf8str(llutf16string((const U16*) w_str));
}
