...
LLUserAuth*	LLUserAuth::mInstance = NULL;

LLUserAuth* LLUserAuth::getInstance()
{
	if( mInstance == NULL )
	{
		mInstance = new LLUserAuth;
	}
	//
	return mInstance;
}

void LLUserAuth::Release()
{
	delete mInstance;
	mInstance = NULL;
}
	...
	const std::string& hashed_volume_serial)
{
	...
	mTransaction.reset( new LLXMLRPCTransaction(auth_uri, request) );
