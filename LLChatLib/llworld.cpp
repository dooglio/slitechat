...
void LLWorld::destroyClass()
{
	...
	{
		LLViewerRegionPtr region_to_delete = *region_it++;
...
LLViewerRegionPtr LLWorld::addRegion(const U64 &region_handle, const LLHost &host)
{
	LLMemType mt(LLMemType::MTYPE_REGIONS);
	
	LLViewerRegionPtr regionp = getRegionFromHandle(region_handle);
	if (regionp)
	{
	...
	LLViewerRegion* reg = new LLViewerRegion( region_handle
										    , host
											, mWidth
											, WORLD_PATCH_SIZE
...
void LLWorld::removeRegion(const LLHost &host)
{
	...
	LLViewerRegionPtr regionp = getRegion(host);
	if (!regionp)
	{
...
LLViewerRegionPtr LLWorld::getRegion(const LLHost &host)
{
	for (region_list_t::iterator iter = mRegionList.begin();
		 iter != mRegionList.end(); ++iter)
	{
		LLViewerRegionPtr regionp = *iter;
		if (regionp->getHost() == host)
		{
			return regionp;
		}
	}
	return LLViewerRegionPtr();
}

#if 0
LLViewerRegionPtr LLWorld::getRegionFromPosAgent( const LLVector3 &pos)
{
	return getRegionFromPosGlobal( LLC::ManagerImpl::GetAgentId().getPosGlobalFromAgent(pos));
}

LLViewerRegionPtr LLWorld::getRegionFromPosGlobal(const LLVector3d &pos)
{
	for (region_list_t::iterator iter = mRegionList.begin();
		 iter != mRegionList.end(); ++iter)
	{
...
LLVector3d	LLWorld::clipToVisibleRegions(const LLVector3d &start_pos, const LLVector3d &end_pos)
{
	...
	LLViewerRegionPtr regionp = getRegionFromPosGlobal(start_pos);
	if (!regionp) 
	{
		return start_pos;
	}
...
LLViewerRegionPtr LLWorld::getRegionFromHandle(const U64 &handle)
{
	for (region_list_t::iterator iter = mRegionList.begin();
		 iter != mRegionList.end(); ++iter)
	{
...
LLViewerRegionPtr LLWorld::resolveRegionGlobal(LLVector3 &pos_region, const LLVector3d &pos_global)
{
...
LLViewerRegionPtr LLWorld::resolveRegionAgent( LLVector3 &pos_region, const LLVector3 &pos_agent )
{
	LLVector3d pos_global = LLC::ManagerImpl::GetAgentId().getPosGlobalFromAgent(pos_agent);
	LLViewerRegion *regionp = getRegionFromPosGlobal(pos_global);
...
void LLWorld::printPacketsLost()
{
		 ...
		 iter != mActiveRegionList.end(); ++iter)
	{
		...
		{
#if 0
			LLVector3d range = regionp->getCenterGlobal() - LLC::ManagerImpl::GetAgentId().getPositionGlobal();
...
{
	LLViewerRegionPtr region = LLWorld::getInstance()->getRegion(msg->getSender());
	if( region )
	{
							...
							) const
{
		...
		iter != LLWorld::getInstance()->getRegionList().end(); ++iter)
	{
		...
		S32 count = regionp->GetMapAvatars().count();
		for (S32 i = 0; i < count; i++)
		{
			LLVector3d pos_global = unpackLocalToGlobalPosition(regionp->GetMapAvatars().get(i), origin_global );
			...
			if( distance <= radius )
			{
				...
				if(avatar_ids != NULL)
				{
					avatar_ids->push_back(regionp->GetMapAvatarIDs().get(i));
				}
			}
		}
	}
...
{
	std::cout << "process_enable_simulator()" << std::endl;
	...
	msg->addUUIDFast(_PREHASH_SessionID, LLC::ManagerImpl::GetInstance()->GetSessionId());
	msg->addUUIDFast(_PREHASH_ID, LLC::ManagerImpl::GetInstance()->GetAgentId());
...
public:
	...
	{
		std::cout << "LLEstablishAgentCommunication::post() handled" << std::endl;
		...
		LLViewerRegionPtr regionp = LLWorld::getInstance()->getRegion(sim);
		if (!regionp)
		{
			std::cout << "Got EstablishAgentCommunication for unknown region "
					<< sim << std::endl;
			return;
		}
...
{	
	std::cout << "process_disable_simulator()" << std::endl;
...
{
	std::cout << "process_region_handshake()" << std::endl;

	LLHost host = msg->getSender();
	LLViewerRegionPtr regionp = LLWorld::getInstance()->getRegion(host);
	if (!regionp)
	{
